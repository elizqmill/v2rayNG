...
sealed class RealPingEvent {
    ...
    /** A single server download-speed result is available (bytes per second, -1 on failure). */
    data class SpeedResult(val guid: String, val speedBytesPerSec: Long) : RealPingEvent()
